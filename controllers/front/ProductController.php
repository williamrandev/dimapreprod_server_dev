...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    public function initContent(): void
    {
        if (!$this->errors) {
            ...
            // Debut $liste_fichiers = scandir("ERP/IN/");
            $scandir_liste_fichiers = scandir(_PS_ROOT_DIR_."/ERP/IN/");
            $this->liste_fichiers = $scandir_liste_fichiers;
                $tabf = array();
                $tabf2 = array();
                $tabfins = array();
                if($scandir_liste_fichiers && count($scandir_liste_fichiers) > 0) {
                    for ($i = 0; $i < count($this->liste_fichiers); $i++) {
                        if ($this->liste_fichiers[$i] != "." && $this->liste_fichiers[$i] != "..") {
                            $tab_fic = explode("_", $this->liste_fichiers[$i]);                       
                            if (count($tab_fic) >= 3) {
                                // $versionfunc = intval($tab_fic[0]);
                                $identifiant = $this->filtre_fichier_aux1(substr($tab_fic[1], 1));
                                $img = $tab_fic[2]; 
                                // echo var_dump($img);  
                                // die();                         
                                //echo "<br/>".$version." ".$identifiant." ".$img."<br/><br/>";
                                //echo "<br/>i=".$i." , version=".$version." , lettre=".$tab_fic[1][0]." , identifiant=".$identifiant." , id=".$id."<br/>";
                                // var_dump(intval($identifiant));
                                // var_dump($this->product->id - 100);
                                if ($tab_fic[1][0] == "P" && intval($identifiant) == ($this->product->id - 100) && substr($img, 0, 2) == "LG") {
                                    $tabf[] = "https://" . $_SERVER['HTTP_HOST'] . "/ERP/IN/" . urlencode(utf8_encode($this->liste_fichiers[$i]));
                                }
        
                                if ($tab_fic[1][0] == "P" && intval($identifiant) == ($this->product->id - 100) && substr($img, 0, 3) == "INS") {
                                    $tabfins[] = "https://" . $_SERVER['HTTP_HOST'] . "/ERP/IN/" . urlencode(utf8_encode($this->liste_fichiers[$i]));
                                }
            ...
            //fin $liste_fichiers = scandir("ERP/IN/");
    ...
    //enleve les 0 au d̩but d'un string
    function filtre_fichier_aux1($chaine)
    {
        while ($chaine[0] == "0") {
