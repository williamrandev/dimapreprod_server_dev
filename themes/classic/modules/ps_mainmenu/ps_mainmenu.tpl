    ...
    {if $nodes|count}
      <ul class="top-menu container sub_menu_edit{$depth}" {if $depth == 0}id="top-menu"{/if} data-depth="{$depth}">
        {foreach from=$nodes item=node}
            <li class="{$node.type}{if $node.current} current {/if}{if isset($node.clickable) && !$node.clickable} no-link{/if}" id="{$node.page_identifier}">
            {assign var=_counter value=$_counter+1}
              <a
                class="{if $depth >= 0}dropdown-item{/if}{if $depth === 1} dropdown-submenu{/if}{if isset($node.clickable) && !$node.clickable} disabled-link{/if}"
                href="{if isset($node.clickable) && !$node.clickable}javascript:void(0);{else}{$node.url}{/if}" data-depth="{$depth}"
                {if $node.open_in_new_window} target="_blank" {/if}
                {if isset($node.clickable) && !$node.clickable} onclick="event.preventDefault(); return false;" style="cursor: default;"{/if}
              >
      ...
      {function name="menu_custom" nodes=[] depth=0 parent=null}
        {if $nodes|count}
          <ul class="custom-menu-list container custom_sub_menu_{$depth}" {if $depth == 0}id="custom-menu-root"{/if} data-depth="{$depth}">
            {foreach from=$nodes item=node}
              <li class="custom-{$node.type}{if $node.current} current {/if}{if isset($node.clickable) && !$node.clickable} no-link{/if}" id="custom_{$node.page_identifier}">
                {assign var=_counter_custom value=$_counter_custom+1}
                <a
                  class="{if $depth >= 0}custom-dropdown-item{/if}{if $depth === 1} custom-dropdown-submenu{/if}{if isset($node.clickable) && !$node.clickable} disabled-link{/if}"
                  href="{if isset($node.clickable) && !$node.clickable}javascript:void(0);{else}{$node.url}{/if}" data-depth="{$depth}"
                  {if $node.open_in_new_window} target="_blank" {/if}
                  {if isset($node.clickable) && !$node.clickable} onclick="event.preventDefault();"{/if}
                >
                  {if $node.children|count}
      ...
      <div class="menu custom-js-top-menu position-static" id="custom_menu_wrapper">
        <li class="close_menu_mobile">
          ...
          <div class="pull-right"> 
            ...
            {if $language.iso_code == 'fr'}Retour{else}Back{/if}
          </div>
        </li>
