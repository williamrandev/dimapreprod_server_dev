  ...
  <div class="form-group row {if !empty($field.errors)}has-error{/if}">
    ...
    <div class="col-md-6 js-input-column{if ($field.type === 'radio-buttons')} form-control-valign{/if}">
      ...
      {elseif $field.type === 'birthday'}
       ...
       <div class="input-group js-parent-focus" style="position: relative;">
            <input
              id="field-{$field.name}"
            ...
            <span class="input-group-btn" style="position: absolute; right: 0; top: 0; height: 100%; z-index: 10;">
              <button
                class="btn"
                type="button"
                 aria-label="{l s='Show / Hide password' d='Shop.Theme.Actions'}"
                 style="height: 100%; background: transparent; border: none; padding: 0 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #7a7a7a;"
                onclick="var inp=this.closest('.input-group').querySelector('input'); var isPass=inp.type==='password'; inp.type=isPass?'text':'password'; this.querySelector('.eye-open').style.display=isPass?'none':'block'; this.querySelector('.eye-closed').style.display=isPass?'block':'none';">
