  ...
  <section id="main">
    ...
    <div class="row product-container js-product-container">
      ...
      <div class="pb-left-column col-md-5 ">
              ...
              function obsShowDesc10(select, text) {
                if ($('.obsAttrRadio').length == 0) {
                  if ($('#obsAttrLgGroup_' + $(select).attr('name')).length)
                    ...
                    $('.logos-produit').append('<div id="obsAttrLgGroup_' + $(select).attr('name') +
                      '" class="obsDescElement">' + text + '</div>');
                }
              }
        ...
        <div class="logos-produit2">
          {foreach from=$logo_ins item=logo_in}
            <div>
              <img src="{$logo_in}" />
            </div>
          {/foreach}
      ...
      <div class="col-md-7">
        <div class="col-md-8">
          {assign var="nom" value=$product.name}
          {assign var="longueur" value=$nom|strlen}
          {assign var="moitie" value=$longueur/2|round}

          {assign var="premiere_partie" value=$nom|substr:0:$moitie}
          {assign var="deuxieme_partie" value=$nom|substr:$moitie}

          {assign var="nom_avec_br" value=$premiere_partie|cat:"<br>"|cat:$deuxieme_partie}

          {block name='page_header_container'}
            {block name='page_header'}
              <h1 class="h1">
                {block name='page_title'}
                  {$nom_avec_br nofilter}
                {/block}
              </h1>
            {/block}
          {/block}

          <p class="Ref">REF : {$product.reference_to_display}</p>
...
<div class="product-actions js-product-actions">
  {block name='product_buy'}
    <form action="{$urls.pages.cart}" method="post" id="add-to-cart-or-refresh">
      ...
      {* 1. LES PIÈCES D'IDENTITÉ (Indispensable pour le panier) *}
      ...
      {* 3. LES VARIANTES (Taille, Couleur, etc.) *}
      {block name='product_variants'}
        {include file='catalog/_partials/product-variants.tpl'}
      {/block}

      {* 4. LE BOUTON AJOUTER AU PANIER *}
