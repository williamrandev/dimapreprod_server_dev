...
{*
      {if !$configuration.is_catalog}
     <a class="col-lg-4 col-md-6 col-sm-6 col-xs-12" id="quotes-link" href="{url entity='module' name='pricequote' controller='pricequote'}">
          <span class="link-item">
            <i class="material-icons">description</i>
            {if (isset($lang_iso) && $lang_iso == 'fr') || (isset($language) && $language.iso_code == 'fr')}
              {l s='Mes devis' d='Shop.Theme.Customeraccount'}
            {else}
              {l s='My quotes' d='Shop.Theme.Customeraccount'}
            {/if}
          </span>
        </a>
      {/if}
